...
@class OCFPageViewController;

/*
 *  OCFPage 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (OCFPageCachePolicyLowMemory)，并在一段时间后切换到 High .
 收到多次警告后，会停留在到 OCFPageCachePolicyLowMemory 不再增长
 *
 ...
 */
typedef NS_ENUM(NSInteger, OCFPageCachePolicy) {
    OCFPageCachePolicyDisabled   = -1,  // Disable Cache
    OCFPageCachePolicyNoLimit    = 0,   // No limit
    OCFPageCachePolicyLowMemory  = 1,   // Low Memory but may block when scroll
    OCFPageCachePolicyBalanced   = 3,   // Balanced ↑ and ↓
    OCFPageCachePolicyHigh       = 5    // High
};

typedef NS_ENUM(NSUInteger, OCFPagePreloadPolicy) {
    OCFPagePreloadPolicyNever     = 0, // Never pre-load controller.
    OCFPagePreloadPolicyNeighbour = 1, // Pre-load the controller next to the current.
    OCFPagePreloadPolicyNear      = 2  // Pre-load 2 controllers near the current.
};

NS_ASSUME_NONNULL_BEGIN
@protocol OCFPageViewControllerDataSource <NSObject>
@required
...
/**
 ...
 @param pageViewController The container controller
...
/**
 *  To inform how many child controllers will in `OCFPage`.
 *
 *  @param pageViewController The parent controller.
 *
 *  @return The value of child controllers's count.
 */
- (NSInteger)numbersOfChildControllersInPageViewController:(OCFPageViewController *)pageViewController;
 ...
 */
- (UIViewController *)pageViewController:(OCFPageViewController *)pageViewController viewControllerAtIndex:(NSInteger)index;

/**
 *  Each title you wanna show in the `OCFPageBaseMenuView`
 *
 *  @param pageViewController The parent controller.
 *  @param index          The index of title.
 *
 *  @return A `NSString` value to show at the top of `OCFPage`.
 */
- (NSString *)pageViewController:(OCFPageViewController *)pageViewController titleAtIndex:(NSInteger)index;

@end

@protocol OCFPageViewControllerDelegate <NSObject>
@optional
 ...
 *
 *  @param pageViewController The parent controller (OCFPage)
 ...
 */
- (void)pageViewController:(OCFPageViewController *)pageViewController lazyLoadViewController:(UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageViewController:(OCFPageViewController *)pageViewController willCachedViewController:(UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageViewController:(OCFPageViewController *)pageViewController willEnterViewController:(UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageViewController:(OCFPageViewController *)pageViewController didEnterViewController:(UIViewController *)viewController withInfo:(NSDictionary *)info;

@end

@interface OCFPageViewController : OCFScrollViewController <UIScrollViewDelegate, OCFPageMenuViewDataSource, OCFPageMenuViewDelegate, OCFPageViewControllerDataSource, OCFPageViewControllerDelegate>
/**
 ...
 */
@property (nonatomic, strong, nullable) NSMutableArray<NSString *> *keys;
@property (nonatomic, strong, nullable) NSMutableArray<id> *values;
 ...
 */
@property (nonatomic, assign) OCFPageMenuViewStyle menuViewStyle;

@property (nonatomic, assign) OCFPageMenuViewLayout menuViewLayout;
...
@property (nonatomic, assign) OCFPageCachePolicy cachePolicy;

/** 预加载机制，在停止滑动的时候预加载 n 页 */
@property (nonatomic, assign) OCFPagePreloadPolicy preloadPolicy;
...
/** 顶部导航栏 */
@property (nonatomic, weak, nullable) OCFPageBaseMenuView *menuView;

/** 内部容器 */
@property (nonatomic, weak, nullable) OCFPageScrollView *pageScrollView;

/** MenuView 内部视图与左右的间距 */
@property (nonatomic, assign) CGFloat menuViewContentMargin;

@property (nonatomic, weak) id<OCFPageViewControllerDataSource> dataSource;
@property (nonatomic, weak) id<OCFPageViewControllerDelegate> delegate;
